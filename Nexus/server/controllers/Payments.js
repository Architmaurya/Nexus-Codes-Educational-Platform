...
exports.directEnroll = async (req, res) => {
  ...
  if (!courses || courses.length === 0 || !userId) {
    return res.status(400).json({
      success: false,
      message: "Please provide Course ID(s) and User ID",
    });
  }
  ...
  try {
    for (const courseId of courses) {
      ...
      const isAlreadyEnrolled = course.studentsEnroled.includes(userId);
    ...
    return res.status(200).json({
      success: true,
      message: "Successfully enrolled in course(s) without payment",
    });
  } catch (error) {
